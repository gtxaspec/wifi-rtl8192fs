 ...
 *
 *****************************************************************************/
#define _RTL8192FS_XMIT_C_

#include <rtl8192f_hal.h>

#if defined( DBG_TX_DSC)
...
query_free_page:
	...
	if (rtl8192f_lps_poff_get_txbndy_status(padapter) == _TRUE) {
	...
	for (idx = 0; idx < hwentry; idx++) {
		...
		while (rtw_end_of_queue_search(sta_phead, sta_plist) == _FALSE) {
			...
			while (rtw_is_list_empty(frame_phead) == _FALSE) {
				...
				if ((NULL == pxmitbuf) ||
					...
					if (pxmitbuf) {
						...
						if (pxmitbuf->len > 0 && pxmitbuf->priv_data) {
							...
							pframe->agg_num = k;
							pxmitbuf->agg_num = k;
							rtl8192f_update_txdesc(pframe, pframe->buf_addr);
							rtw_free_xmitframe(pxmitpriv, pframe);
							pxmitbuf->priv_data = NULL;
				...
				if (ret == _FAIL) {
					...
					k++;
					if (k != 1)
						rtl8192f_update_txdesc(pxmitframe, pxmitframe->buf_addr);
					rtw_count_tx_stats(padapter, pxmitframe, pxmitframe->attrib.last_txcmdsz);
					pre_qsel = pxmitframe->attrib.qsel;
					txlen = txdesc_size + pxmitframe->attrib.last_txcmdsz;
					pxmitframe->pg_num =PageNum(txlen, PAGE_SIZE_TX_8192F);
					pxmitbuf->pg_num += PageNum(txlen, PAGE_SIZE_TX_8192F);
					/* if (k != 1) */
		...
		if (pxmitbuf) {
			...
			if (pxmitbuf->len > 0) {
				...
				rtl8192f_update_txdesc(pframe, pframe->buf_addr);
				#if defined( DBG_TX_DSC)
				_dbg_dump_tx_info(padapter, pframe->frame_tag, pframe->buf_addr);
				#endif/*DBG_TX_DSC*/
				rtw_free_xmitframe(pxmitpriv, pframe);
				pxmitbuf->priv_data = NULL;
				enqueue_pending_xmitbuf(pxmitpriv, pxmitbuf);
 ...
 *	_FAIL		something error
 */
s32 rtl8192fs_xmit_handler(PADAPTER padapter)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
	_irqL irql;
...
thread_return rtl8192fs_xmit_thread(thread_context context)
{
	...
	do {
		ret = rtl8192fs_xmit_handler(padapter);
...
s32 rtl8192fs_mgnt_xmit(PADAPTER padapter, struct xmit_frame *pmgntframe)
{
	...
	rtl8192f_update_txdesc(pmgntframe, pmgntframe->buf_addr);

	pxmitbuf->len = txdesc_size + pattrib->last_txcmdsz;
	pxmitbuf->pg_num = PageNum(pxmitbuf->len , PAGE_SIZE_TX_8192F); 
...
#ifdef CONFIG_RTW_MGMT_QUEUE 
s32 rtl8192fs_hal_mgmt_xmitframe_enqueue(PADAPTER adapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv;
	s32 ret;
