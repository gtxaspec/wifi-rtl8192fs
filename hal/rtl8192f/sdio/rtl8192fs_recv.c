...
s32 rtl8192fs_recv_hdl(_adapter *padapter)
{
	...
	do {
		...
		while (ptr < precvbuf->ptail) {
			...
			/* rx desc parsing */
			rtl8192f_query_rx_desc_status(precvframe, ptr);
...
static void rtl8192fs_recv_tasklet(void *priv)
{
	_adapter *adapter = (_adapter *)priv;
	s32 ret;

	ret = rtl8192fs_recv_hdl(adapter);
...
s32 rtl8192fs_init_recv_priv(PADAPTER padapter)
{
	...
	tasklet_init(&precvpriv->recv_tasklet,
		     (void(*)(unsigned long))rtl8192fs_recv_tasklet,
